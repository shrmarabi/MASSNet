...
def makeplot(rs, ps, outDir, class_name, iou_type):
    ...
    for i in range(len(areaNames)):
        area_ps = ps[..., i, 0]
        file_title = iou_type + '-' + class_name + '-' + areaNames[i]
        ...
        with open(outDir + f'{file_title}.txt', 'w') as file:
            for k in range(len(types)):
                file.write(f"Type: {types[k]}, AP: {aps[k]:.3f}\n")
                for r_val, p_val in zip(rs, ps_curve[k + 1]):
                    file.write(f"Recall: {r_val}, Precision: {p_val}\n")
                file.write("\n")
